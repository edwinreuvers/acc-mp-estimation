...
def replace_latex_table_cell(latex_str: str, row: int, col: int, new_text: str) -> str:
    """
...
def fix_reference(text: str, citation_map: dict[str, str]) -> str:
    """
    Replace @Key references in the text with \citeproc{Key}{Custom text}, using a mapping.

    Parameters:
        text (str): The original text containing @Key references.
        citation_map (dict): Dictionary where keys are reference keys (e.g., 'DeJong1900'),
                             and values are the custom citation texts.

    Returns:
        str: Updated text with \citeproc{}{} replacements.
    """
    def replacer(match):
        key = match.group(1)
        if key in citation_map:
            return f"\\citeproc{{ref-{key}}}{{{citation_map[key]}}}"
        else:
            return match.group(0)  # Leave it unchanged if not found
